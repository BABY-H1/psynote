...
| BUG-007 | MAJOR | 已修(待 commit, Phase H 深度修 + Phase J IA 重构) | 研判分流详情面板 3 按钮在无规则机构永远 disabled. **Phase H 真正修复**: (1) 新加 `POST /triage/results/:id/candidate` lazy-create 端点(sourceRuleId=null 标记手工创建, 幂等防重复) (2) `useLazyCreateCandidate` hook + TriageActionBar `ensureCandidate` ensure-then-act (3) workflow accept 扩展 episode_candidate kind 真创建 careEpisode + 返回 episodeId (mirror crisis pattern). **Phase J IA 重构**: 危机处置全流程 (清单 5 步 + 提交督导 + 签字 + 结案) 整个留在研判中心右栏 inline 渲染 CrisisChecklistPanel, 不跳工作台 (用户心智模型: 分诊台先做必做项, 不是马上跳进长期咨询). 已结案 row list 显示"危机处置中"角标 + 点开右栏顶部出现"已结案"绿色 banner + "开后续咨询 episode" 按钮. ✅ |
| BUG-007-VISUAL | MINOR | 已修(待 commit, Phase J) | 研判分流 L3/L4 row selected 态时 level 暖色 (橙/红) 跟 brand-50 冷色叠层产生色差冲突. 修法: TriageCandidateList row selected 时左竖条改 brand 色实色, 整行加 ring-2 ring-brand-300, 不动 level badge (语义保留). 视觉断言验证 selected row bg=rgb(236, 253, 245) brand-50 + ring 生效. ✅ |

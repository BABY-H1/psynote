...
export function AdminLibrarySchemes() {
  const [items, setItems] = useState<Scheme[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [distTarget, setDistTarget] = useState<Scheme | null>(null);
  ...
  async function loadItems() {
    setLoading(true);
    try {
      const data = await api.get<Scheme[]>('/admin/library/schemes');
      setItems(data);
    } catch (err) {
      console.error('Failed to load system schemes:', err);
    } finally {
      setLoading(false);
    }
  }
    ...
    <div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="px-5 py-12 text-center text-slate-400 text-sm">
            {items.length === 0 ? '暂无系统团辅方案' : '没有匹配的结果'}
          </div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="text-left px-5 py-3 text-xs font-medium text-slate-500">方案名称</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-slate-500">场次</th>
              ...
              {filtered.map((s) => (
                <tr key={s.id} className="hover:bg-slate-50">
                  <td className="px-5 py-3">
                    ...
                    {s.description && <div className="text-xs text-slate-400 mt-0.5 truncate max-w-sm">{s.description}</div>}
                  </td>
                  <td className="px-4 py-3 text-sm text-slate-600">{s.sessionCount || '-'}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${s.visibility === 'public' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
                      {s.visibility === 'public' ? '已发布' : '草稿'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
      ...
      {distTarget && (
        <DistributionEditor
          ...
          onSave={async (orgIds) => {
            await api.patch(`/admin/library/schemes/${distTarget.id}/distribution`, { allowedOrgIds: orgIds });
            await loadItems();
          }}

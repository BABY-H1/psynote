...
import { Settings, FolderPlus, Link2, X, ChevronLeft } from 'lucide-react';
import {
  useEpisodes,
  useAppointments,
...
import { AvailabilitySettings } from '../../counseling/pages/AvailabilitySettings';
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 flex flex-col w-full min-h-0">
        <div className="flex items-center justify-between mb-3 flex-shrink-0">
          <div className="flex items-center gap-3">
            <h3 className="text-base font-bold text-slate-900">
              {schedulerOpen ? '排班设置' : '预约管理'}
            </h3>
            {!schedulerOpen && pendingCount > 0 && (
          ...
          >
            {schedulerOpen ? (
              <>
                <ChevronLeft className="w-3.5 h-3.5" /> 返回预约
              </>
            ) : (
              <>
        ...
        {schedulerOpen ? (
          ...
          <>
            ...
            {/* Grouped list — internal scroll so the page itself doesn't move */}
            <div className="flex-1 min-h-0 overflow-y-auto -mx-1 px-1">
              {grouped.length === 0 ? (
                ...
                <div className="space-y-4">
                  {grouped.map(({ date, label, items }) => (
                    <div key={date}>
                      <h4 className="text-sm font-semibold text-slate-700 mb-2 sticky top-0 bg-white py-1">{label}</h4>
                      <div className="space-y-2">
                        {items.map((item) => (
                          <AppointmentCard
                            ...
                            onCardClick={
                              item.careEpisodeId
                                ? () => navigate(`/episodes/${item.careEpisodeId}`)
                                : undefined
                            }
                          />
                        ))}

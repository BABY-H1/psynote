...
function AssessmentCheckboxList({ assessments, selected, onToggle }: {
  ...
  if (assessments.length === 0) {
    return <p className="text-xs text-slate-400 italic">暂无可用量表，请先在测评管理中创建</p>;
  }
  return (
...
export function AfterPhase({ state, onChange, assessments }: Props) {
  ...
  const toggleAssessment = (field: 'postGroup' | 'satisfaction', id: string) => {
    ...
    const updated = current.includes(id)
      ...
      : [...current, id];
    onChange({
      assessmentConfig: { ...state.assessmentConfig, [field]: updated },
    });
  };

  const addFollowUpRound = () => {
    const lastDelay = followUpRounds.length > 0 ? followUpRounds[followUpRounds.length - 1].delayDays : 0;
    const newRound: FollowUpRound = {
      assessments: [],
      delayDays: lastDelay + 30,
      label: `第${followUpRounds.length + 1}次随访`,
    };
    onChange({
      assessmentConfig: {
        ...state.assessmentConfig,
        followUp: [...followUpRounds, newRound],
      },
    });
  };
  ...
  const updateFollowUpRound = (index: number, patch: Partial<FollowUpRound>) => {
    ...
    onChange({
      assessmentConfig: { ...state.assessmentConfig, followUp: updated },
    });
  };

  const removeFollowUpRound = (index: number) => {
    const updated = followUpRounds.filter((_, i) => i !== index);
    onChange({
      assessmentConfig: { ...state.assessmentConfig, followUp: updated },
    });
  };
  ...
  const toggleFollowUpAssessment = (roundIndex: number, assessmentId: string) => {
    ...
    const updated = round.assessments.includes(assessmentId)
      ? round.assessments.filter((id) => id !== assessmentId)
  ...
  return (
    <div className="space-y-4">
      ...
      <CollapsibleSection
        ...
        defaultOpen
      >
        <p className="text-xs text-slate-400">与入组前测对比，评估团体辅导效果</p>
        <AssessmentCheckboxList
          assessments={assessments}
          selected={state.assessmentConfig.postGroup || []}
          onToggle={(id) => toggleAssessment('postGroup', id)}
        />
      </CollapsibleSection>

      {/* Follow-up Assessment (multiple rounds) */}
      <CollapsibleSection
        title="随访评估"
        icon={<RotateCcw className="w-4 h-4 text-green-500" />}
        subtitle={followUpRounds.length > 0 ? `${followUpRounds.length} 轮` : undefined}
        defaultOpen
      >
        <p className="text-xs text-slate-400 mb-2">结束后按设定天数自动推送给来访者，可配置多轮</p>

        {followUpRounds.length === 0 ? (
          <div className="text-center py-4">
            <p className="text-xs text-slate-400 mb-2">暂未配置随访评估</p>
          </div>
        ) : (
          <div className="space-y-4">
            {followUpRounds.map((round, idx) => (
              <div key={idx} className="bg-slate-50 rounded-lg p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <input
                      value={round.label || ''}
                      onChange={(e) => updateFollowUpRound(idx, { label: e.target.value })}
                      placeholder={`第${idx + 1}次随访`}
                      className="px-2 py-1 border border-slate-200 rounded text-sm bg-white w-32"
                    />
                    <div className="flex items-center gap-1.5">
                      <span className="text-xs text-slate-500">结束后</span>
                      <input
                        type="number"
                        value={round.delayDays}
                        onChange={(e) => updateFollowUpRound(idx, { delayDays: Math.max(1, Number(e.target.value)) })}
                        min={1}
                        className="w-16 px-2 py-1 border border-slate-200 rounded text-sm text-center bg-white"
                      />
                      <span className="text-xs text-slate-500">天</span>
                    </div>
                  </div>
                  <button
                    onClick={() => removeFollowUpRound(idx)}
      ...
      <CollapsibleSection
        ...
        defaultOpen={false}
      >
        <p className="text-xs text-slate-400">收集来访者对活动的整体反馈</p>
        <AssessmentCheckboxList
          assessments={assessments}
          selected={state.assessmentConfig.satisfaction || []}
